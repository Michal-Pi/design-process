...
**What goes wrong:**
22 skills × ~200 chars fits the 2% cap *in isolation* (MRD §5 math: ~4.4k chars). But Codex truncates the aggregate skill-trigger index across **all installed packages**. Once a user installs 5+ other popular skill packages alongside design-os, the index exceeds ~8k chars and trigger descriptions get silently truncated mid-sentence. Skills that triggered cleanly in single-package CI start ghosting in production.
...
- design-os has the largest surface of any common skill package (22 vs. typical 4-8)
...
- **Split design (contingency):** if pressure rises, fork into `design-os-core` (7 workflows only, ~1.4k chars) + `design-os-atoms` (15 atoms as a companion package); MRD §12 already names this lever
...
**Why it happens:**
- LLMs are sycophantic; the NN/g 2024 + ACM Interactions 2026 papers are *exactly* the literature design-os is trying to honor, and they're the same papers showing LLMs cave to social pressure
...
- **PII leak:** interview transcripts contain real participant names, emails, recorded contexts; if a user accidentally commits `research/interviews/`, design-os has now caused a PII breach
...
**How to avoid:**
- **Ship `.gitignore` and `.gitattributes` snippets as installable artifacts** (`design-os install --git-hooks`):
  ...
  - `.design-os/private/` → gitignored
  - `design/*.json merge=ours` in `.gitattributes` for the high-churn JSON files
- **PII scanner pre-commit hook:** `design-os scan --pii` runs as a git pre-commit hook on `design/` paths; refuses commit if a transcript contains email/phone patterns or anything matching a participant-name registry the user maintains
- **Manifest reconciliation script:** `design-os verify --manifest` runs in CI; mismatched `MANIFEST.md` ↔ filesystem state fails the build
...
- **Schema version migration tooling:** when `schemaVersion: 1` becomes `schemaVersion: 2`, `design-os migrate --schema persona` is mandatory and the gate refuses to run with mixed versions
...
- Users report "design-os just stops in the middle of stage 4"
...
- `design-os verify --golden` is in CI but the golden has been regenerated from the latest run instead of from a frozen baseline
...
- **Determinism CI gate:** `design-os verify --golden` runs the full `design` workflow on the fixture 5 times; all 5 emit-script outputs must be byte-identical (LLM picks can vary; emit must not)
...
- `design-os verify --golden` runs but the assertion is "outputs are similar" rather than "outputs are byte-identical"
...
- The `audit --all-stages` cross-stage workflow blows past budget on a project that's been using design-os for months (deep `design/` directory)
...
- Users who installed v2.0a now have `design/` directories with v1 schemas; on `design-os update`, their projects don't upgrade
- A workflow update silently expects a field that v1 didn't have; runs fail with cryptic errors

**How to avoid:**
- **Migration scripts shipped with every schema bump:** `design-os migrate --schema sitemap --from 1 --to 2` is mandatory tooling
...
- A user reports "ran `design-os update`, now my project doesn't work"
...
**What goes wrong:**
MRD §12 names "Anthropic ships a 5-stage equivalent in Claude Design" as Medium-likelihood, **Existential** impact. Claude Design is exactly the right team to do this; they have distribution, training data, brand. If they ship `claude-design-stages` before design-os GA, the wedge is gone.
...
- Designer trust in Anthropic is high; their brand executes the same wedge faster than design-os's distribution

**How to avoid:**
- **Ship faster:** v2.0a (4 stages, weeks 1-8) is end-to-end usable on its own; do not wait for full v2.0 to start distribution
- **Differentiate on host-portability (MRD §12 mitigation):** design-os runs on Cursor, Codex CLI, Junie, Copilot — Claude Design ships on Claude only. The portability story is non-overlapping.
...
- **DESIGN.md compliance and DTCG output:** make design-os the *reference implementation* of these standards; even if Anthropic ships first, design-os interoperates
- **Open-source license (Apache-2.0):** removes a moat Anthropic might choose not to cross; design-os is the OSS canonical version
...
- Brad Frost or NN/g cite Claude Design rather than design-os when discussing stage-aware AI tools
...
- **v2.0b:** Lovable refugee path is unique to design-os (Anthropic unlikely to ship `extract --reverse-engineer` from competitor outputs); double down here
...
**What goes wrong:**
MRD §2.4 is the trust-gap table; every line is a known designer objection to AI design tools. Even with every mitigation in place, design-os can still be *perceived* as a slop-generator because the marketplace is full of bad faith AI design pitches. The result:
...
- **Never use the phrase "AI design":** package name (design-os), all skill names, all docs avoid the term; "design-process," "stage-typed," "validation gates" are the vocabulary
...
**What goes wrong:**
MRD §12 names this High-likelihood, Medium-impact. The 5-stage process *is* heavier than `lovable.dev "build me a landing page"`. Indie devs are process-averse; if their first encounter with design-os is "you must complete Stage 1 interviews before you can generate UI," they uninstall.

**Why it happens:**
- The wedge product (Lovable / v0) delivers instant gratification; design-os deliberately doesn't
- The full `design` workflow at p50 ≤150k tokens / ≤8 minutes wall-clock is real friction
- Indie devs evaluating skills install 5-10 packages and try each for 60 seconds; design-os looks slow
...
|---|---|---|
| **Claude Code (host-first)** | Assume CLAUDE.md tool restrictions apply to skill execution | Skill execution honors host's tool gating; design-os must call out which tools each workflow needs and degrade gracefully |
...
| **Excalidraw JSON format** | Treat as opaque blob | It's MIT-licensed JSON; design-os emits and validates it via script — `excalidraw-validate.mjs` |
| **DTCG v2025.10** | Forward-compat features (slip into v2026.X usage) | Spec-pin to v2025.10; bump = explicit migration PR |
| **Google DESIGN.md spec** | Author the spec ourselves | Cession in MRD §14 — Google owns; design-os emits compliant output |
| **shadcn/ui** | Modify `components/ui/` files directly (per CLAUDE.md) | Wrap, don't modify; design-os generates components into a parallel path or as wrappers |
...
|---|---|---|
| **Committing `research/interviews/*.transcript.md` with real participant PII** | PII breach; legal exposure; trust collapse | `.gitignore` by default; `design-os scan --pii` pre-commit hook; explicit team-private mechanism documented |
| **Logging full `decision-log.jsonl` to a public repo** | Internal design rationale exposed | `.design-os/private/` is gitignored per v1.0.1 commit policy |
...
| **Running `design-os install --git-hooks` without showing what's installed** | Surprise hook installation; user loses control | `install` is opt-in, shows hook content, requires `--yes` confirmation |
...
| **`design-os update` running scripts from untrusted sources** | Supply chain | Package is Apache-2.0; updates go via host (Claude Code / Codex CLI / Cursor) skill-package install path; no autonomous network fetch |
...
|---|---|---|
| **Codex 2% cap breach (aggregate)** | MEDIUM | (1) Audit which skills are firing falsely / not firing; (2) Truncate descriptions for atoms first (workflows take priority); (3) If still failing, split into `design-os-core` + `design-os-atoms` packages per MRD §12 mitigation |
...
- **Aggregate trigger-metadata cap** (Pitfall 1) — MRD R15 has the eval; this pitfall is the *operational reality* of regression once design-os grows or competitors ship overlapping packages
...
---
*Pitfalls research for: design-os v2.0 (SKILL.md package operationalizing the canonical 5-stage design process)*
*Researched: 2026-05-24*
