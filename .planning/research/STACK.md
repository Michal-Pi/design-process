...
**Domain:** SKILL.md package operationalizing the 5-stage design process (Garrett spine: Strategy → Scope → Structure → Skeleton → Surface) inside coding agents (Claude Code / Cursor / Codex / Junie / Copilot).
**Researched:** 2026-05-24
**Confidence:** HIGH on spec/format pinning (verified against Context7 + official sources). MEDIUM on auxiliary tooling (XState v5 designer-receptivity, Excalidraw programmatic generation maturity). LOW on a couple of "is this thing real and stable" items (skillgrade, Codex 2% cap — both treated as known unknowns below).

---

## Stack split (read this first)

The MRD calls for three distinct stacks. Conflating them is the #1 architectural risk.
...
The adapter interface is the same v1.0.1 contract: `port.lock`, `_imports/` symlink, security sandbox, readiness probe.
...
npm install -E zod@^4.4 gray-matter@^4 yaml@^2 ajv@^8 ajv-formats@^3 culori@^4 \
  apca-w3@latest @bjornlu/wcag-contrast@latest semver@^7 globby@^14 pino@^9 \
  commander@^12

# Render + preview
npm install -E mermaid@^11.15 @mermaid-js/mermaid-cli@^11.15 \
  @excalidraw/excalidraw@latest @excalidraw/mermaid-to-excalidraw@latest \
  xstate@^5.20

# Dev
npm install -ED typescript@^5.7 tsx@^4 tsup@^8 vitest@^2 \
  @playwright/test@^1.60 axe-core@^4.11 \
  eslint@^9 @typescript-eslint/eslint-plugin@^8 @typescript-eslint/parser@^8 \
  prettier@^3 @changesets/cli@^2 \
...
**If the user has a Next.js + Tailwind v4 + shadcn repo (the MRD §3.7 W5 acceptance fixture):**
...
**If the user is a Lovable / v0 / Bolt refugee (no local dev-server):**
...
**If the user runs Codex CLI or Cursor (not Claude Code):**
...
6. **XState v5 vs Mermaid scope** — the MRD already says Mermaid is canonical for designers (§3.22, codex finding). The deterministic emitter `state-machine-emit.mjs` should support **both** outputs from a single internal IR. Don't force users to maintain two artifacts in sync; emit both from one.

---

## Sources

**Specs (highest authority)**
...
**Ecosystem / trust posture context**
